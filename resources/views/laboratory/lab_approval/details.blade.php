            ...
            @foreach ($test_parameters as $test_parameter)
                @foreach ($library_test_parameters as $library_test_parameter)
                    @if ($test_parameter->test_parameters === $library_test_parameter->id)
                        @include('laboratory.lab_approval.partials.micro.' . $library_test_parameter->filename, ['micro1'=>$micro1])
                    @endif
                @endforeach
            @endforeach
        </form>
...
{{-- pychem --}}
{{-- @if ($rawDataFile->test_parameters === 'chem')
    <form action="{{ route('laboratory.lab-approval-form.chem', ['analysis_id' => $requests->analysis_id]) }}"
            method="POST">
            @csrf
        @foreach ($test_parameters as $test_parameter)
            @foreach ($library_test_parameters as $library_test_parameter)
